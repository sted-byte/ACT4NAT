...
class LevenshteinTransformerModel(FairseqNATModel):
    ...
    def __init__(self, args, encoder, decoder):
        ...
        # self.linear1 = nn.Linear(512,len(self.tgt_dict))
    ...
    def forward(
        self, src_tokens, src_lengths, prev_output_tokens, tgt_tokens, indicator, src_tokens_after, encoder_mlm_mask,  **kwargs
    ):
        assert tgt_tokens is not None, "forward function only supports training."

        # encoding   ABCDEFG      AB[MASK]
        encoder_out = self.encoder(src_tokens, src_lengths=src_lengths, indicator=indicator, **kwargs)
        with_multi_task = False
        bert_out = None
        if src_tokens_after != None:
            with_multi_task = True
            assert encoder_mlm_mask is not None
        if with_multi_task:
            bert_out = self.encoder(src_tokens_after,src_lengths=src_lengths, indicator=indicator, **kwargs)
            mlm_outputs = self.linear1(bert_out['encoder_out'][0].transpose(0,1))
            real_inputs = mlm_outputs[encoder_mlm_mask]
            real_target = src_tokens[encoder_mlm_mask]
            mlm_loss = self.loss_func(real_inputs, real_target)
